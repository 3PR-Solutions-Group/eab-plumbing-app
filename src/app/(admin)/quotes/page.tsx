...
type Quote = { id: string; customerName: string; template: string; items: QuoteItem[]; total: number; status: string; date: string };

function parseQuote(row: any): Quote {
  let customerName = "", template = "";
  try { const p = JSON.parse(row.notes || "{}"); customerName = p.customerName || ""; template = p.template || ""; } catch {}
  return {
    id: row.id,
    customerName: customerName || "—",
    template,
    items: Array.isArray(row.line_items) ? row.line_items : [],
    total: row.total || 0,
...
export default function QuotesPage() {
  ...
  const [form, setForm] = useState({ customerName: "", template: "", items: [] as QuoteItem[] });

  useEffect(() => {
    supabase.from("quotes").select("*").order("created_at", { ascending: false })
      .then(({ data }) => { if (data) setQuotes(data.map(parseQuote)); setLoading(false); });
  }, []);
  ...
  const submit = async () => {
    ...
    const num = (count ?? 0) + 1;
    const quote_number = `QUO-${String(num).padStart(3, "0")}`;
    ...
    const { data, error } = await supabase.from("quotes").insert({
      ...
      total: grandTotal,
      notes: JSON.stringify({ customerName: form.customerName, template: form.template }),
    }).select().single();

    if (data && !error) setQuotes(qs => [parseQuote(data), ...qs]);
  ...
  if (showing) return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 mb-5 flex justify-between">
        ...
        <span className="font-bold text-lg text-blue-700">£{grandTotal.toFixed(2)}</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {TEMPLATES.map(t => (
          <button key={t.name} onClick={() => { setForm({ customerName: "", template: t.name, items: t.items.map(i => ({ ...i })) }); setShowing(true); }}
            ...
            <div className="text-xs text-blue-600 mt-0.5">from £{t.items.reduce((s, i) => s + i.quantity * i.unitPrice, 0)}</div>
          </button>
        ))}
      </div>
      {loading ? <div className="text-center text-gray-400 py-4">Loading…</div> : quotes.length > 0 && (
